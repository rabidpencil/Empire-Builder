...
function makeCore(G, CARDS, CITIES){
  ...
  function steiner(terms,owned,wf){
    const W=wf||((u,v,sg)=>w(u,v,sg,owned));
    ...
    for(let m=1;m<F;m++){
      ...
      while(pq.size){ const [c,u]=pq.pop(); if(c>row[u])continue;
        for(const [v,sg] of adj[u]){ const nc=c+W(u,v,sg); if(nc<row[v]){row[v]=nc;par[m][v]=['e',u];pq.push([nc,v]);} } }
    }
  ...
  function connect(aKey,bKey,owned,fewestMoves){
    owned=owned||new Set();
    const wt=fewestMoves ? (()=>1) : ((u,v,sg)=>w(u,v,sg,owned));
    ...
    while(pq.size){ const [c,u]=pq.pop(); if(c>d[u])continue; if(u===dst)break;
      for(const [v,sg] of adj[u]){ const nc=c+wt(u,v,sg); if(nc<d[v]){d[v]=nc;prev[v]=u;pq.push([nc,v]);} } }
    ...
    return {cost:treeCost(new Set(edges),owned,src),moves:edges.length,edges,newSegments:fresh.length};
  }
